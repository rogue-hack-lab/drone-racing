...
/*
* 2016OCT17 - MODIFIED FOR USE WITH PARTICLE.IO DEVICES - <kevin.r.conner@gmail.com>
*/
...
{
  uint32_t threshold = time ? time : holdEventThreshold; //use holdEventThreshold if time == 0
	//should we trigger a onHold event?
  if (pressedStartTime!=-1 && !triggeredHoldEvent) 
  {
...
*/
void Button::holdHandler(buttonEventHandler handler, uint32_t holdTime /*=0*/)
{
...
*/
uint32_t Button::holdTime() const { if (pressedStartTime!=-1) { return millis()-pressedStartTime; } else return 0; }
